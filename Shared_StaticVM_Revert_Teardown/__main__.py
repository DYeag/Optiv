...
def main():
    ...
    if len(static_vms) > 0:
        for static_vm in static_vms:
            if sandbox.automation_api.GetResourceLiveStatus(resourceFullPath=static_vm.Name).liveStatusName != "Offline":
                if static_vm.Shared:
                    ...
                    if res_count == 1:
                        ...
                        try:
                            ...
                            sandbox.automation_api.WriteMessageToReservationOutput(
                                reservationId=sandbox.id,
                                message='Could not find specified snapshot for {}'.format(static_vm.Name)
                            )
                        sandbox.automation_api.ExecuteResourceConnectedCommand(
                            reservationId=sandbox.id,
                            resourceFullPath=static_vm.Name,
